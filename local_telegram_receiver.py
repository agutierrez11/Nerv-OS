...
def search_company_file(name):
    ...
    vaults = [
        Path("/home/antonio/Desktop/Incode_WarRoom_Vault"),
        Path("/home/antonio/Desktop/Ecosistema_Fintech_Global")
    ]
...
def search_battlecard_file(name):
    ...
    competencia_dirs = [
        ...
        Path("/home/antonio/Desktop/Ecosistema_Fintech_Global/Empresas")
    ]
...
def get_sefaria_context(query, max_results=3):
    sefaria_keywords = [
        "sefaria", "pele yoetz", "pele yoets", "talmud", "torah", "tora", "mishnah", "misna", "gemara",
        "tanaj", "tanakh", "shuljan", "shulchan", "rambam", "maimonides", "rashi", "zohar", "judaismo",
        "judio", "hebreo", "bible", "biblia", "torat", "musar", "mussar", "halaj", "halakh", "siddur", "sidur"
    ]
    query_lower = query.lower()
    is_sefaria_related = any(kw in query_lower for kw in sefaria_keywords)
    
    if not is_sefaria_related:
        return ""
        
    try:
        # Generate optimal English search query using DeepSeek
        client = DeepSeekClient()
        translation_prompt = f"""Analiza la consulta del usuario en español y genera un término de búsqueda en inglés (de 1 a 3 palabras) optimizado para buscar en la base de datos de Sefaria.org (Textos tradicionales judíos como Tanaj, Talmud, Musar, Pele Yoetz, etc.).
        
Ejemplos:
- "Pele Yoetz sobre el enojo" -> "Pele Yoetz Anger"
- "que dice el Talmud del amor" -> "Talmud Love"
- "Shuljan Aruj sobre Shabat" -> "Shulchan Arukh Shabbat"
- "Pele Yoetz soberbia" -> "Pele Yoetz Pride"
- "que es el Pele Yoetz" -> "Pele Yoetz"

Consulta del usuario: "{query}"

Responde ÚNICAMENTE con el término de búsqueda en inglés, sin explicaciones ni comillas.
"""
        messages = [
            {"role": "system", "content": "Eres un asistente experto en traducción y optimización de búsquedas para la biblioteca de Sefaria."},
            {"role": "user", "content": translation_prompt}
        ]
        
        response = client.create_completion(model="deepseek-chat", messages=messages, temperature=0.1)
        sefaria_query = response.choices[0].message.content.strip().replace('"', '')
        
        logger.info(f"Sefaria RAG: Query traducido a '{sefaria_query}'")
        ...
        search_resp = requests.post(search_url, headers=headers, json=search_payload, timeout=12)
        if search_resp.status_code != 200:
            logger.error(f"Sefaria Search API returned status {search_resp.status_code}")
            return ""
            
        search_data = search_resp.json()
        hits = search_data.get("hits", {}).get("hits", [])
        if not hits:
            logger.info("No se encontraron resultados en Sefaria.")
            return ""
            
        context_pieces = []
        for hit in hits:
            full_id = hit.get("_id", "")
            # Extract ref (e.g., "Pele Yoetz 342:1" from "Pele Yoetz 342:1 (Sefaria Community Translation [en])")
            ...
            # Fetch text details
            text_url = f"https://www.sefaria.org/api/texts/{ref}?context=0"
            logger.info(f"Sefaria RAG: Obteniendo texto para '{ref}'")
            txt_resp = requests.get(text_url, timeout=10)
            if txt_resp.status_code == 200:
                txt_data = txt_resp.json()
                
                # Get English text
                en_text = txt_data.get("text", "")
                if isinstance(en_text, list):
                    en_text = "\n".join([t for t in en_text if isinstance(t, str)])
                    
                # Get Hebrew text
                he_text = txt_data.get("he", "")
                if isinstance(he_text, list):
                    he_text = "\n".join([t for t in he_text if isinstance(t, str)])
                
                # Clean HTML tags if any
                en_text = re.sub(r'<[^>]*>', '', en_text)
                he_text = re.sub(r'<[^>]*>', '', he_text)
                
                # Format
                piece = f"--- Sefaria Source: {ref} ---\n"
                if he_text.strip():
                    piece += f"[Hebrew]: {he_text.strip()}\n"
                if en_text.strip():
                    piece += f"[English]: {en_text.strip()}\n"
        ...
        logger.error(f"Error en get_sefaria_context: {e}")
        return ""
...
def run_general_chat(chat_id, query):
    ...
    try:
        ...
        # Sefaria Integration
        sefaria_context = get_sefaria_context(query)
        if sefaria_context:
            context_pieces.append(sefaria_context)
            send_telegram_message(chat_id, "📚 *Sefaria:* Textos tradicionales encontrados y añadidos al contexto.")
        ...
        query_words = [w.lower().strip() for w in re.split(r'\W+', query) if len(w.strip()) > 3]
        if not query_words:
        ...
        for vault in vaults:
            ...
            for md_file in vault.rglob("*.md"):
                ...
                if any(word in file_stem_lower for word in query_words):
...
Si la información no es suficiente, responde con tu conocimiento general sobre la industria fintech, antifraude, KYB/KYC y tecnologías de identidad, o sobre textos tradicionales judíos, filosofía y ética de la tradición (como el Pele Yoetz, Talmud, etc.), dependiendo de la naturaleza de la consulta.

CONTESTA DE FORMA CONCISA, ESTRUCTURADA Y CON TONO EJECUTIVO/DE NEGOCIOS (o un tono de estudio y sabiduría respetuoso si te preguntan sobre temas de Torá/Musar/Sefaria). Usa viñetas y negritas para mejorar la legibilidad.

CONTEXTO LOCAL Y SEFARIA DISPONIBLE:
{context_str}
...
"""
        messages = [
            {"role": "system", "content": "Eres el Core de Inteligencia de NERV OS, un experto en ciberseguridad, identidad digital, prevención de fraude, y también un asistente de estudios con acceso a la biblioteca de textos judíos Sefaria."},
